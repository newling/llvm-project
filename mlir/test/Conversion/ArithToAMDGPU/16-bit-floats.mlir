...
func.func @vector_trunc_long(%v: vector<9xf32>) -> vector<9xf16> {
  ...
  // CHECK: %[[out0:.*]] = vector.insert_strided_slice %[[packed0]], {{.*}} {offsets = [0]} : vector<2xf16> into vector<9xf16>
  ...
  // CHECK: %[[out1:.*]] = vector.insert_strided_slice %[[packed1]], %[[out0]] {offsets = [2]} : vector<2xf16> into vector<9xf16>
  ...
  // CHECK: %[[out2:.*]] = vector.insert_strided_slice %[[packed2]], %[[out1]] {offsets = [4]} : vector<2xf16> into vector<9xf16>
  // CHECK: %[[elem6:.*]] = vector.extract %[[value]]
  // CHECK: %[[elem7:.*]] = vector.extract %[[value]]
  // CHECK: %[[packed3:.*]] = rocdl.cvt.pkrtz %[[elem6]], %[[elem7]] : vector<2xf16>
  // CHECK: %[[out3:.*]] = vector.insert_strided_slice %[[packed3]], %[[out2]] {offsets = [6]} : vector<2xf16> into vector<9xf16>
  // CHECK: %[[elem8:.*]] = vector.extract %[[value]]
  // CHECK: %[[packed4:.*]] = rocdl.cvt.pkrtz %[[elem8:.*]] : vector<2xf16>
  // CHECK: %[[slice:.*]] = vector.extract_strided_slice %[[packed4]] {offsets = [0], sizes = [1]} : vector<2xf16> to vector<1xf16>
  // CHECK: %[[out4:.*]] = vector.insert_strided_slice %[[slice]], %[[out3]] {offsets = [8]} : vector<1xf16> into vector<9xf16>
  // CHECK: return %[[out4]]
