...
gpu.module @kernels {
  ...
  gpu.func @kernel0(%arg0: vector<5xf16>) kernel {
    ...
    // CHECK-SUB: %[[E0:.+]] = vector.extract_strided_slice %[[ARG0]] {offsets = [0], sizes = [2]} : vector<5xf16> to vector<2xf16>
    // CHECK-SUB: %[[R0:.+]] = gpu.subgroup_reduce add %[[E0]] : (vector<2xf16>) -> vector<2xf16>
    // CHECK-SUB: %[[V0:.+]] = vector.insert_strided_slice %[[R0]], %[[VZ]] {offsets = [0]} : vector<2xf16> into vector<5xf16>
    // CHECK-SUB: %[[E1:.+]] = vector.extract_strided_slice %[[ARG0]] {offsets = [2], sizes = [2]} : vector<5xf16> to vector<2xf16>
    // CHECK-SUB: %[[R1:.+]] = gpu.subgroup_reduce add %[[E1]] : (vector<2xf16>) -> vector<2xf16>
    // CHECK-SUB: %[[V1:.+]] = vector.insert_strided_slice %[[R1]], %[[V0]] {offsets = [2]} : vector<2xf16> into vector<5xf16>
  ...
  gpu.func @kernel6(%arg0: vector<3xi8>) kernel {
    ...
    // CHECK-SHFL: %[[V0:.+]] = vector.insert_strided_slice %[[ARG0]], %[[CZ]] {offsets = [0]} : vector<3xi8> into vector<4xi8>
    ...
    // CHECK-SHFL: %[[ESS:.+]] = vector.extract_strided_slice %{{.+}} {offsets = [0], sizes = [3]} : vector<4xi8> to vector<3xi8>
