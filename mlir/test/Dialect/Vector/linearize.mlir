...
func.func @test_extract_strided_slice_2D(%arg0 : vector<4x8xf32>) -> vector<2x2xf32> {
  ...
  // CHECK: return %[[RES]] : vector<2x2xf32
  %0 = vector.extract_strided_slice %arg0 { sizes = [2, 2], offsets = [0, 4]}
...
func.func @test_extract_strided_slice_2D_scalable(%arg0: vector<4x[8]xf32>) -> vector<2x[8]xf32> {
  ...
  %0 = vector.extract_strided_slice %arg0 { sizes = [2, 8], offsets = [1, 0] } : vector<4x[8]xf32> to vector<2x[8]xf32>
...
func.func @test_extract_strided_slice_3D(%arg0 : vector<2x8x2xf32>) -> vector<1x4x2xf32> {
  ...
  %0 = vector.extract_strided_slice %arg0 { offsets = [1, 2], sizes = [1, 4] }
...
//  CHECK-SAME:      [0, 1, 2, 3, 4, 5, 12, 13, 14, 15, 10, 11] : vector<12xi8>, vector<4xi8>
  %0 = vector.insert_strided_slice %arg0, %arg1 {offsets = [1, 0, 0, 0]} : vector<2x2xi8> into vector<2x1x3x2xi8>
...
//  CHECK-SAME:       [0, 1, 2, 3, 4, 5, 6, 7, 8, 18, 10, 19, 12, 13, 14, 15, 16, 17] : vector<18xi8>, vector<2xi8>
  %0 = vector.insert_strided_slice %arg0, %arg1 {offsets = [1, 1, 1], sizes = [1, 2, 1]} : vector<1x2x1xi8> into vector<3x3x2xi8>
...
func.func @insert_strided_slice_2D_higher_offsets(%arg0 : vector<2x1xi8>, %arg1 : vector<2x2xi8>, %arg2 : vector<5x2xi8>) -> vector<5x2xi8> {
  ...
  //                     ^^^ ^^^ ^^^ ^^^
  //                    insertion indices
  %0 = vector.insert_strided_slice %arg1, %arg2 {offsets = [2, 0], sizes = [2, 2]} : vector<2x2xi8> into vector<5x2xi8>

  // CHECK: [0, 1, 2, 3, 10, 5, 11, 7, 8, 9]
  //                     ^^^    ^^^
  %1 = vector.insert_strided_slice %arg0, %0 {offsets = [2, 0], sizes = [2, 1]} : vector<2x1xi8> into vector<5x2xi8>

  // CHECK: [0, 1, 2, 3, 4, 5, 6, 10, 8, 11]
  //                              ^^^    ^^^
  %2 = vector.insert_strided_slice %arg0, %1 {offsets = [3, 1], sizes = [2, 1]} : vector<2x1xi8> into vector<5x2xi8>
...
func.func @negative_insert_strided_slice_scalable(%arg0 : vector<1x[2]xi8>, %arg1 : vector<2x[2]xi8>) -> vector<2x[2]xi8> {
  %0 = vector.insert_strided_slice %arg0, %arg1 {offsets = [0, 0]} : vector<1x[2]xi8> into vector<2x[2]xi8>
